...
from pytego import Pytego, _RuleSet


class TestPytego(TestCase):
    def test_allowed(self):
        ...
        rp = Pytego.parse(content=content)
    ...
    def test_allowed_wildcards(self):
        ...
        self.assertFalse(rp.can_fetch("https://www.site.local/pytegoworks", "first"))
        self.assertFalse(rp.can_fetch("https://www.site.local/pytego_works", "first"))
        self.assertFalse(rp.can_fetch("https://www.site.local/pytego/works", "first"))
        self.assertFalse(
            rp.can_fetch("https://www.site.local/pytego_does_work", "first")
        )
        self.assertTrue(
            rp.can_fetch("https://www.site.local/rotego_does_work", "first")
        )
        self.assertTrue(rp.can_fetch("https://www.site.local/pytego_does_wor", "first"))
    ...
    def test_generosity(self):
        ...
        Foo: Foobot
        Bar: /"""
        rp = Pytego.parse(content=robotstxt_incorrect)
        ...
        request rate 1/10s 1820-1940
        """
        rp = Pytego.parse(content=robotstxt_incorrect_accepted)
        ...
        disallow: /myprofile
        """
        rp = Pytego.parse(content=wildcards_in_user_agent)
    ...
    def test_bytestrings(self):
        ...
        with self.assertRaises(ValueError) as context:
            Pytego.parse(content=content)

        self.assertEqual("Pytego.parse expects str, got bytes", str(context.exception))
